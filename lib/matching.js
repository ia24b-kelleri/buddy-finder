...
function findeBuddies(ich, alleUser, maxVorschlaege = MAX_VORSCHLAEGE) {
  ...
  const maxScore =
    GEWICHT_FACH * (meinProfil.faecher ? meinProfil.faecher.length : 0) +
    GEWICHT_INTERESSE * (meinProfil.interessen ? meinProfil.interessen.length : 0) +
    GEWICHT_LERNZEIT * (meinProfil.lernzeiten ? meinProfil.lernzeiten.length : 0);
  ...
  return alleUser
    ...
    .map(u => {
      ...
      return {
        ...
        status: (u.profil && u.profil.status) || 'verfügbar',
        gemeinsameFaecher,
        gemeinsameInteressen,
        zeitUeberlappungen,
        score,
        prozent: maxScore > 0 ? Math.min(100, Math.round((score / maxScore) * 100)) : 0,
      };
    })
    .filter(b => b.score > 0)
